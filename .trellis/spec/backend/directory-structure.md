...
- `veritas/run_logging.py` owns local run-output helpers: output base
  ...
  cache-use manifest construction, and saved MinerU URL/ZIP artifacts. It should
  stay filesystem-local and must not call
  providers.
...
- `veritas/web_runner.py` owns local Web Runner helper functions that do not
  ...
  path picking, dropped file URI parsing, Web Runner audit subprocess command
  preparation, and namespace-aware config/default-output helpers.
