...
def test_extract_cache_payload_records_stage1_cache_fields(tmp_path):
    ...
    payload = paper_audit.extract_cache_payload(
        input_path,
        cache_version=7,
        use_mineru=True,
        mineru_lang="ch",
...
def test_save_llm_failure_cache_result_persists_payload_and_event(tmp_path):
    saved = {}
    events = []
    chunk_cache = tmp_path / "chunk_0001.json"

    def fake_json_save(path, payload):
        saved["path"] = path
    ...
    paper_audit.save_llm_failure_cache_result(
        chunk_cache,
        RuntimeError("schema bad"),
        chunk_idx=1,
        total_chunks=3,
        status="failed_final",
        json_save=fake_json_save,
        resume_event_func=fake_resume_event,
        resume_dir=tmp_path / "resume",
        first_error="timeout",
    )

    assert saved["path"] == chunk_cache
    assert saved["payload"]["report"] == {"parse_error": True, "raw_output": "schema bad"}
    assert saved["payload"]["raw_content"] == "schema bad"
    assert saved["payload"]["chunk_index"] == 1
    assert saved["payload"]["total_chunks"] == 3
    assert saved["payload"]["status"] == "failed_final"
    assert saved["payload"]["first_error"] == "timeout"
    assert isinstance(saved["payload"]["saved_at"], str)
    assert events == [
        (
            tmp_path / "resume",
            "stage3_llm_chunk",
            "failed_final",
            "chunk=2/3; error=schema bad",
            {"cache": str(chunk_cache)},
        )
    ]


def test_llm_retry_start_summary_builds_failed_chunk_numbers_and_event_detail():
    summary = paper_audit.llm_retry_start_summary(
        [("chunk-a", 0, "timeout"), ("chunk-c", 2, "schema bad")],
        llm_cache_only=True,
    )

    assert summary == {
        "failed_chunks": [1, 3],
        "event_detail": "failed_chunks=[1, 3]; cache_only=True",
    }
...
def test_package_boundaries_export_existing_compatibility_surface():
    ...
    assert veritas.mineru_text._extract_mineru_structured_text is paper_audit._extract_mineru_structured_text
    ...
    assert veritas.report_checks._is_suspicious_check is paper_audit._is_suspicious_check
    assert veritas.report_checks._check_sort_key is paper_audit._check_sort_key
    ...
    assert veritas.review_overview.build_audit_action_items is paper_audit.build_audit_action_items
    assert veritas.review_overview.format_audit_action_summary_html is paper_audit.format_audit_action_summary_html
    assert veritas.review_overview.format_audit_action_summary_markdown is paper_audit.format_audit_action_summary_markdown
    assert veritas.review_overview.build_review_overview is paper_audit.build_review_overview
    assert veritas.review_overview.format_review_overview_html is paper_audit.format_review_overview_html
    assert veritas.review_overview.format_review_overview_markdown is paper_audit.format_review_overview_markdown
    ...
    assert veritas.reference_parsing._crossref_work_to_match is paper_audit._crossref_work_to_match
    assert veritas.reference_parsing._openalex_work_to_match is paper_audit._openalex_work_to_match
    assert veritas.reference_parsing._pubmed_summary_to_match is paper_audit._pubmed_summary_to_match
    ...
    assert veritas.artifacts._failed_artifact_options is paper_audit._failed_artifact_options
    ...
    assert veritas.failed_diagnostics.format_failed_audit_markdown is paper_audit.format_failed_audit_markdown
    assert veritas.failed_diagnostics.format_failed_audit_html is paper_audit.format_failed_audit_html
    assert veritas.failed_diagnostics.save_failed_audit_diagnostics is paper_audit.save_failed_audit_diagnostics
...
def test_web_runner_page_contains_workbench_controls():
    ...
    assert paper_audit.web_runner_page_head_markup() in rendered
    ...
    assert "chooseLocalPath" in paper_audit.web_runner_page_input_script()
    assert "renderReportPanel" in paper_audit.web_runner_page_report_script()
    assert "artifactLinks" in paper_audit.web_runner_page_report_script()
    assert "startRunWithPayload" in paper_audit.web_runner_page_run_script()
    assert "pollLogs" in paper_audit.web_runner_page_run_script()
